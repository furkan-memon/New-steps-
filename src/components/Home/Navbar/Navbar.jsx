...
const [panel, setPanel] = useState(null);
  ...
  <header className="top-0 left-0 relative w-full">
   <div className="fixed top-0 left-0 w-full z-[1004] 
...
px-4 py-4">
          ...
          return (
            <NavLink
              ...
              className={({ isActive }) =>
                ...
                ${isActive ? "text-[#0F3558]" : "text-gray-600 hover:text-[#2FA4D6]"}`
              }
            >
              {names[index]}

              {/* underline */}
              <span className="absolute left-0 bottom-0 h-[2px] w-full bg-[#2FA4D6]
              origin-right scale-x-0 transition-transform duration-300
              group-hover:origin-left group-hover:scale-x-100"></span>
            </NavLink>
          );
        })}
      </ul>
  ...
  <Link to="/Wishlist">
    <Heart size={22} className="hover:text-[#2FA4D6] transition" />
  </Link>
